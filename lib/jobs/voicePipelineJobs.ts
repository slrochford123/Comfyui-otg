...
export type LtxBackendSubmissionState = "pre_submit" | "submitted" | "submission_unknown" | "failed" | "completed";
...
export type QueuedContractJob = {
  ...
  submissionState?: LtxBackendSubmissionState | null;
};
...
function publicJob(job: StoredQueuedContractJob): QueuedContractJob {
  return {
    ...
    preferredGpu: job.preferredGpu ?? null,
    actualGpu: job.actualGpu ?? null,
    backend: job.backend ?? null,
    fallbackReason: job.fallbackReason ?? null,
    backendEndpoint: job.backendEndpoint ?? null,
    backendService: job.backendService ?? null,
    submissionId: job.submissionId ?? null,
    promptId: job.promptId ?? null,
    submissionState: job.submissionState ?? null,
  };
}
...
function normalizeInvalidRunningJob(job: StoredQueuedContractJob, now: Date): StoredQueuedContractJob | null {
  ...
  if (job.action === "generate_training_dataset" && generatedClipCount(job) <= 0) {
    ...
    return {
      ...
      message: releasedFromLocalWorker
        ? "Dataset job was released from the local development worker. Waiting for the Linux IndexTTS2 dataset worker."
        : "Queued / waiting for Linux IndexTTS2 dataset worker.",
      error: null,
  ...
  const localDatasetMessage = isLocalDatasetWorkerLease(job)
    ? "Dataset job was released from the local development worker. Resume with the Linux IndexTTS2 dataset worker."
...
export function createCharacterVoicePipelineJob(ownerKey: string, rawInput: unknown): JobValidationResult {
  ...
  if (action === "create_voice_sample") {
    if (rawInput.provider === "ltx" || rawInput.provider === "unnatural_ltx") {
      job = updateVoicePipelineJob(ownerKey, job.jobId, {
        preferredGpu: "3090",
        actualGpu: null,
        backend: null,
        fallbackReason: null,
...
function qwen3Provider(value: unknown): boolean {
  const provider = cleanString(value).toLowerCase();
  return provider === "qwen3" || provider === "qwen3-tts" || provider === "qwen3_tts";
}

export function isQwen3CreateVoiceSampleJob(job: Pick<QueuedContractJob, "jobType" | "action" | "input">): boolean {
  return (
    job.jobType === "character_voice_pipeline" &&
    job.action === "create_voice_sample" &&
    qwen3Provider(job.input?.provider || job.input?.voiceProvider || job.input?.method)
  );
}

export function hasOtherActiveQwen3VoiceSampleJobs(exceptJobId: string): boolean {
  const normalizedExceptJobId = cleanString(exceptJobId);
  const activeStatuses = new Set<QueuedContractJobStatus>(["queued", "running"]);
  return readStoreWithFreshLeases().jobs.some((job) =>
    job.jobId !== normalizedExceptJobId &&
    activeStatuses.has(job.status) &&
    isQwen3CreateVoiceSampleJob(job)
  );
}
...
export function updateVoicePipelineJob(ownerKey: string, jobId: string, patch: VoicePipelineJobUpdate): QueuedContractJob | null {
  ...
  const next: StoredQueuedContractJob = {
    ...
    preferredGpu: patch.preferredGpu === undefined ? current.preferredGpu ?? null : patch.preferredGpu,
    actualGpu: patch.actualGpu === undefined ? current.actualGpu ?? null : patch.actualGpu,
    backend: patch.backend === undefined ? current.backend ?? null : patch.backend,
    fallbackReason: patch.fallbackReason === undefined ? current.fallbackReason ?? null : patch.fallbackReason,
    backendEndpoint: patch.backendEndpoint === undefined ? current.backendEndpoint ?? null : patch.backendEndpoint,
    backendService: patch.backendService === undefined ? current.backendService ?? null : patch.backendService,
    submissionId: patch.submissionId === undefined ? current.submissionId ?? null : patch.submissionId,
    promptId: patch.promptId === undefined ? current.promptId ?? null : patch.promptId,
    submissionState: patch.submissionState === undefined ? current.submissionState ?? null : patch.submissionState,
  };
...
function ltxRoutingPatch(value: unknown): VoicePipelineJobUpdate {
  if (!isPlainObject(value)) return {};
  const preferredGpu = value.preferredGpu === "3090" ? "3090" : undefined;
  const actualGpu = value.actualGpu === "3090" || value.actualGpu === "5060-ti" ? value.actualGpu : value.actualGpu === null ? null : undefined;
  const backend = value.backend === "primary" || value.backend === "fallback" ? value.backend : value.backend === null ? null : undefined;
  const allowedStates: LtxBackendSubmissionState[] = ["pre_submit", "submitted", "submission_unknown", "failed", "completed"];
  const submissionState = allowedStates.includes(value.submissionState as LtxBackendSubmissionState)
    ? value.submissionState as LtxBackendSubmissionState
    : undefined;
  return {
    ...(preferredGpu !== undefined ? { preferredGpu } : {}),
    ...(actualGpu !== undefined ? { actualGpu } : {}),
    ...(backend !== undefined ? { backend } : {}),
    ...(Object.prototype.hasOwnProperty.call(value, "fallbackReason") ? { fallbackReason: cleanString(value.fallbackReason) || null } : {}),
    ...(Object.prototype.hasOwnProperty.call(value, "backendEndpoint") ? { backendEndpoint: cleanString(value.backendEndpoint) || null } : {}),
    ...(Object.prototype.hasOwnProperty.call(value, "backendService") ? { backendService: cleanString(value.backendService) || null } : {}),
    ...(Object.prototype.hasOwnProperty.call(value, "submissionId") ? { submissionId: cleanString(value.submissionId) || null } : {}),
    ...(Object.prototype.hasOwnProperty.call(value, "promptId") ? { promptId: cleanString(value.promptId) || null } : {}),
...
function remoteWorkerActionLabel(action: RemoteVoicePipelineWorkerAction): string {
  if (action === "start_applio_training") return "Linux Applio training worker";
  if (action === "test_trained_voice") return "Linux Applio inference worker";
  if (action === "generate_character_preview") return "Linux character preview worker";
  return "Linux IndexTTS2 dataset worker";
}
...
export function checkpointRemoteWorkerJob(
  ...
  return updateVoicePipelineJob(ownerKey, jobId, {
    ...
    progress,
    message: cleanString(message) || current.message || "Remote worker checkpoint.",
    result: mergeJobResult(current.result, {
      ...resultPatch,
      status: "running",
      checkpointAt: nowIso,
    ...
    interruptedAt: null,
    ...ltxRoutingPatch(resultPatch),
  });
}

export function requeueRemoteWorkerJob(
  ownerKey: string,
  jobId: string,
  message?: string,
  result?: unknown,
  ...
  const resultPatch =
    result !== undefined && isPlainObject(result)
      ? sanitizeValue(result) as Record<string, unknown>
      : {};
  ...
  return updateVoicePipelineJob(ownerKey, jobId, {
    ...
    promptId: null,
    submissionState: "pre_submit",
    fallbackReason: normalizedMessage,
    ...
    result: mergeJobResult(current.result, {
      ...
      submissionState: "pre_submit",
      requeuedAt: now,
      requeueReason: normalizedMessage,
    }),
  });
}
...
export function failRemoteTrainingDatasetJob(
  ...
  return failRemoteWorkerJob(ownerKey, jobId, error || "Linux IndexTTS2 training dataset failed.", result);
}
...
export function failRemoteWorkerJob(
  ...
  const normalizedError = cleanString(error) || "Remote worker failed.";
  ...
  if (result !== undefined) {
    ...
    Object.assign(patch, ltxRoutingPatch(result));
  }
