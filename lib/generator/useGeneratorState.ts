...
function normalizeState(input: any): GeneratorState {
  ...
  const enhanceOk =
    s.enhanceLevel === "small"
    || s.enhanceLevel === "medium"
    || s.enhanceLevel === "large";
  ...
  return {
    ...
    presetId:
      typeof s.presetId === "string"
        ? s.presetId
        : defaultGeneratorState.presetId,
    prompt:
      typeof s.prompt === "string"
        ? s.prompt
        : defaultGeneratorState.prompt,
    negative:
      typeof s.negative === "string"
        ? s.negative
        : defaultGeneratorState.negative,
    ratio:
      ratioOk
        ? s.ratio
        : defaultGeneratorState.ratio,
    size:
      sizeOk
        ? s.size
        : defaultGeneratorState.size,
    seconds:
      secondsOk
        ? s.seconds
        : defaultGeneratorState.seconds,
    enhanceLevel:
      enhanceOk
        ? s.enhanceLevel
        : defaultGeneratorState.enhanceLevel,
  };
}
