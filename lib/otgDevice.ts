...
export async function getDeviceIdFromRequest(req: NextRequest): Promise<string> {
  ...
  try {
    ...
    if (ct.includes("application/json")) {
      // Parse a clone so device-id discovery never consumes the route's
      // original JSON body. Routes such as /api/backgrounds authenticate
      // before reading their own request body.
