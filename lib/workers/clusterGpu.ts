...
export async function detectShawnExternalOccupancy(deps: OccupancyDependencies = {}): Promise<ShawnOccupancy> {
  ...
  try {
    ...
    // Qwen may be intentionally stopped so ComfyUI can own the RTX 3090.
    // A transport-level failure to /api/ps is therefore not sufficient to
    // classify the GPU as externally occupied after the process probe has
    // already confirmed that Qwen Code is not running. If the endpoint does
    // respond, however, keep failing closed on a non-2xx response and keep
    // treating a resident model as external occupancy.
    let qwenResponse: Response | null = null;
    {
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 2_000);
      try {
        qwenResponse = await fetcher(`${SHAWN_QWEN_URL}/api/ps`, {
          cache: "no-store",
    ...
    // reachable and idle before the 3090 can be leased.
    {
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 2_000);
      try {
        const comfyResponse = await fetcher(`${SHAWN_COMFY_URL}/queue`, {
          cache: "no-store",
          signal: controller.signal,
        });
        if (!comfyResponse.ok) {
