...
export default function ThreeDPocPage() {
  ...
  const processedPreview = result?.processedImageUrl || status?.processedImageUrl || "";
  const preprocessChanged = Boolean(result?.preprocessChanged ?? status?.preprocessChanged);
  const preprocessConfidence = result?.preprocessConfidence ?? status?.preprocessConfidence ?? null;
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gap: 18 }}>
        <div style={cardStyle()}>
          ...
          <div style={{ marginTop: 8, color: "rgba(255,255,255,0.72)", lineHeight: 1.5 }}>
            Upload one image, send it through the Trellis 2 textured 3D pipeline, and return a downloadable GLB. Background removal is on by default and the page now shows the exact preprocessed image that was actually sent into the 3D pipeline.
          </div>
          <div style={{ marginTop: 12, color: "rgba(255,255,255,0.58)", fontSize: 13 }}>
            Route: <span style={{ color: "#67e8f9" }}>/app/3d-poc</span>
          </div>
        </div>
          ...
          <div style={{ ...cardStyle(), display: "grid", gap: 14, alignContent: "start" }}>
            ...
            {status ? (
              <div style={{ borderRadius: 16, background: "rgba(255,255,255,0.04)", padding: 14, fontSize: 14, lineHeight: 1.6 }}>
                ...
                {status.processedImageUrl ? <div><b>Processed image:</b> ready</div> : null}
                {typeof preprocessConfidence === "number" ? <div><b>Cutout confidence:</b> {confidenceText(preprocessConfidence)}</div> : null}
          ...
          <div style={{ ...cardStyle(), minHeight: 520 }}>
            <div style={{ display: "grid", gap: 18, gridTemplateColumns: "minmax(240px, 360px) minmax(0, 1fr)" }}>
              <div style={{ display: "grid", gap: 18 }}>
                <div>
                  <div style={{ fontWeight: 800, marginBottom: 10 }}>Original input</div>
                  <div style={{ borderRadius: 18, border: "1px solid rgba(255,255,255,0.12)", overflow: "hidden", minHeight: 220, background: "rgba(255,255,255,0.03)" }}>
                    {filePreviewUrl ? (
                      <img src={filePreviewUrl} alt="3D upload source" style={{ width: "100%", display: "block", objectFit: "contain", maxHeight: 320 }} />
                    ) : (
                ...
                <div>
                  <div style={{ fontWeight: 800, marginBottom: 10 }}>
                    Preprocessed input
                    {processedPreview ? (
                      <span style={{ marginLeft: 10, fontSize: 12, color: preprocessChanged ? "#86efac" : "rgba(255,255,255,0.55)" }}>
                        {preprocessChanged ? "background removed" : "fallback/original used"}
                      </span>
                    ) : null}
                  </div>
                  <div style={{ borderRadius: 18, border: "1px solid rgba(255,255,255,0.12)", overflow: "hidden", minHeight: 220, background: "linear-gradient(45deg, rgba(255,255,255,0.06) 25%, transparent 25%), linear-gradient(-45deg, rgba(255,255,255,0.06) 25%, transparent 25%), linear-gradient(45deg, transparent 75%, rgba(255,255,255,0.06) 75%), linear-gradient(-45deg, transparent 75%, rgba(255,255,255,0.06) 75%), rgba(255,255,255,0.03)", backgroundSize: "20px 20px", backgroundPosition: "0 0, 0 10px, 10px -10px, -10px 0px" }}>
                    {processedPreview ? (
                      <img src={processedPreview} alt="Preprocessed Trellis input" style={{ width: "100%", display: "block", objectFit: "contain", maxHeight: 320 }} />
                    ) : (
                      <div style={{ padding: 24, color: "rgba(255,255,255,0.5)" }}>The exact image sent to Trellis will appear here after submit.</div>
                    )}
                  </div>
                </div>
              </div>
