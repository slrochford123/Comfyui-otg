...
export default function AppPageClient({ initialUser = null }: { initialUser?: InitialAppUser }) {
  ...
  const authenticatedOwnerKey = useMemo(() => {
    if ((whoamiQuery.error as (Error & { status?: number }) | null)?.status === 401) return "";
    if (whoamiQuery.data?.authenticated === false) return "";
    return String(
      whoamiQuery.data?.username ||
