...
function extractOutputPathFromProcessCompleted(payload: any, index?: number) {
  ...
  const checkItem = (item: any): string => {
    if (!item || typeof item !== "object") return "";

    if (typeof item.path === "string" && item.path.toLowerCase().endsWith(".glb")) {
      return item.path;
    }
    ...
    if (
      ...
      typeof item.value.path === "string" &&
      item.value.path.toLowerCase().endsWith(".glb")
    ) {
  ...
  if (typeof index === "number") {
    const exact = checkItem(data[index]);
...
function findLatestWhiteMesh(cacheRoot: string, minMtimeMs: number) {
  ...
  for (const entry of entries) {
    ...
    if (!best || mtimeMs > best.mtimeMs) {
      const htmlPath = fs.existsSync(path.join(folder, "white_mesh.html"))
        ? path.join(folder, "white_mesh.html")
        : null;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const savedSourceName = `${Date.now()}_${sanitizeName(path.basename(uploaded.fileName || "upload")) || "upload"}${
      path.extname(uploaded.fileName) ? "" : sourceExt
    }`;
        ...
        return NextResponse.json(
          {
            ...
            directOutputPath: baseOutputPath,
