...
export async function GET(req: NextRequest) {
  ...
  const anglesModelsRoot = path.resolve(path.join(dataRoot, "tmp", "angles_models"));
  const allowed =
    isWithin(storyboardRoot, resolved) ||
    isWithin(anglesPreviewRoot, resolved) ||
    isWithin(anglesModelsRoot, resolved);
  ...
  if (!(ct.startsWith("image/") || ct.startsWith("model/") || /\.(obj|ply)$/i.test(ext))) {
    return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });
  }
