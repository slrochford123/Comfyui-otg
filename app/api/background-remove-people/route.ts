...
async function loadSourceImage(request: NextRequest, sourceValue: string) {
  ...
  if (sourceUrl.origin !== request.nextUrl.origin || !ALLOWED_SOURCE_PROXY_PATHS.has(sourceUrl.pathname)) {
    throw new Error("Selected Background source is not an allowed owner-scoped image reference.");
  }
  const response = await fetch(sourceUrl, {
    cache: "no-store",
    headers: {
      cookie: String(request.headers.get("cookie") || ""),
      "x-otg-device-id": String(request.headers.get("x-otg-device-id") || ""),
    },
  });
  if (!response.ok) {
    throw new Error(`Could not read selected Background source (${response.status}).`);
  }
  const contentType = String(response.headers.get("content-type") || "").split(";")[0].trim();
  if (!contentType.toLowerCase().startsWith("image/")) {
    throw new Error("Selected Background source returned non-image content.");
  }
  return {
    bytes: Buffer.from(await response.arrayBuffer()),
    filename: cleanFilename(sourceValue) || `background-source${contentType.includes("jpeg") ? ".jpg" : contentType.includes("webp") ? ".webp" : ".png"}`,
    contentType,
    sourcePath: "",
  };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.scope !== "user" || !session.username) {
    ...
    const positivePrompt =
      ...
      "Remove all people, humans, persons, crowds, faces, bodies, and human figures. Fill removed areas naturally. Preserve the exact environment, architecture, furniture, lighting, scene composition, camera position, camera angle, background design, materials, color palette, perspective, and overall art style.";
    ...
    const source = await loadSourceImage(request, sourceValue);
    ...
    const promptId = String(queued.json?.prompt_id || "").trim();
    if (!promptId) {
      return NextResponse.json(
        {
          ok: false,
          marker: "OTG_BACKGROUND_REMOVE_PEOPLE_ROUTE_V36AC",
          error: "Qwen Image Edit accepted the request but returned no prompt ID; no replacement was claimed.",
        },
        { status: 502 },
      );
    }
    ...
    const status = error instanceof SessionInvalidError ? 401 : 500;
    return NextResponse.json(
      {
        ok: false,
        marker: "OTG_BACKGROUND_REMOVE_PEOPLE_ROUTE_V36AC",
