...
async function persistBackgroundReference(args: {
  ...
  const originalName = resolved.sourceName || fileNameFromReference(source, args.role);
...
function recordNeedsCanonicalAssets(ownerKey: string, record: BackgroundRecordV36B) {
  ...
  return !stableAssetFromExistingReference(ownerKey, display) || !stableAssetFromExistingReference(ownerKey, workflow);
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const requested = (body?.background || body) as BackgroundRecordInputV36B;
