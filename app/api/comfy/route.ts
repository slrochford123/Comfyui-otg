...
async function peekWorkflowDescriptor(req: NextRequest) {
  ...
  if (!ct.includes("multipart/form-data")) {
    try {
      ...
      return { preset: "", label: "", workflowId: "", workflowLabel: "", requestKind: "", mediaType: "", mode: "" };
    }
  }
...
export async function POST(req: NextRequest) {
  ...
  const COMFY_BASE_URL = route.baseUrl;
  ...
  const skipSizeOverride = isLtxVideoWorkflow(body, graph) || animeImagesWorkflow || krea2TurboWorkflow;
  if (!skipSizeOverride) {
  ...
  const skipGenericFrameTimingOverrideV36BPU31 =
    animeImagesWorkflow || isProductionAnimateBranchSpecificTimingWorkflowV36BPU31(body);
