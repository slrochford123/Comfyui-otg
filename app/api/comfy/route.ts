...
import { ComfyGpuBusyError, submitComfyPromptWith5060Lease } from "@/lib/workers/comfyPromptLease";
...
function setSeedAuto(graph: any) {
  if (!graph || typeof graph !== "object") return { seed: null as number | null };

  // PROD execution contract: every submission gets a fresh server seed.
...
async function peekWorkflowDescriptor(req: NextRequest) {
  ...
  if (!ct.includes("multipart/form-data")) {
    try {
      ...
      return {
        ...
        gpuTarget: String(body?.gpuTarget || "").trim(),
        videoLoras: parsePostedVideoLoras(body?.videoLoras),
      };
    } catch {
      return { preset: "", label: "", workflowId: "", workflowLabel: "", requestKind: "", mediaType: "", mode: "", gpuTarget: "", videoLoras: null };
    }
  }
  ...
  try {
    ...
    return {
      ...
      gpuTarget: String(fd.get("gpuTarget") || "").trim(),
...
const BACKGROUND_MASTER_SEEDVR_WORKFLOW_ID_V36B = "internal/character-reference/seedvr2_character_reference_1080p";
const BACKGROUND_MASTER_SEEDVR_WORKFLOW_FILE_V36B = "comfy_workflows/internal/character-reference/seedvr2_character_reference_1080p.json";
const BACKGROUND_MASTER_SEEDVR_LOAD_IMAGE_NODE_ID_V36B = "1";
const BACKGROUND_MASTER_SEEDVR_SAVE_IMAGE_NODE_ID_V36B = "9";
const BACKGROUND_MASTER_SEEDVR_GPU_TARGET_V36B = "rtx3090";

function normalizeBackgroundMasterSeedvrSelectorV36B(value: unknown) {
  return String(value || "")
    .trim()
    .replaceAll("\\", "/")
    .replace(/^\.\//, "")
...
function isBackgroundMasterSeedvrRequestV36B(body: any) {
  const context = [
    body?.requestKind,
    body?.sourceType,
    body?.galleryExclusionPolicy,
    body?.outputLibrary,
    body?.title,
  ]
    .map((value) => String(value || "").toLowerCase())
  ...
  return context.includes("background-master-seedvr") || context.includes("background-master-only");
}

function assertBackgroundMasterSeedvrRequestV36B(body: any) {
  if (!body || typeof body !== "object" || !isBackgroundMasterSeedvrRequestV36B(body)) return null;

  const workflowId = normalizeBackgroundMasterSeedvrSelectorV36B(body?.workflowId || body?.preset || body?.id);
  const preset = normalizeBackgroundMasterSeedvrSelectorV36B(body?.preset || body?.workflowId || body?.id);
  const workflowSelectors = [
    body?.workflowFile,
    body?.workflowPath,
    body?.workflowJsonPath,
    body?.workflowPresetPath,
  ].map(normalizeBackgroundMasterSeedvrSelectorV36B);
  const expectedWorkflowFile = BACKGROUND_MASTER_SEEDVR_WORKFLOW_FILE_V36B.toLowerCase();
  const usesExpectedWorkflowId =
    workflowId === BACKGROUND_MASTER_SEEDVR_WORKFLOW_ID_V36B ||
    preset === BACKGROUND_MASTER_SEEDVR_WORKFLOW_ID_V36B;
  const usesExpectedWorkflowFile = workflowSelectors.some(
    (selector) => selector === expectedWorkflowFile || selector.endsWith(`/${expectedWorkflowFile}`)
  );
  const loadImageNodeId = String(body?.loadImageNodeId || body?.imageNodeId || "").trim();
  const saveImageNodeId = String(body?.saveImageNodeId || "").trim();
  const gpuTarget = String(body?.gpuTarget || "").trim().toLowerCase();

  if (
    !usesExpectedWorkflowId ||
    !usesExpectedWorkflowFile ||
    loadImageNodeId !== BACKGROUND_MASTER_SEEDVR_LOAD_IMAGE_NODE_ID_V36B ||
    saveImageNodeId !== BACKGROUND_MASTER_SEEDVR_SAVE_IMAGE_NODE_ID_V36B ||
    gpuTarget !== BACKGROUND_MASTER_SEEDVR_GPU_TARGET_V36B
  ) {
    return {
      ok: false,
      code: "BACKGROUND_MASTER_SEEDVR_CONTRACT_MISMATCH",
      error:
        "Background Master SeedVR must use the SeedVR2 1080p workflow on RTX 3090 with LoadImage node 1 and SaveImage node 9.",
      expected: {
        workflowId: BACKGROUND_MASTER_SEEDVR_WORKFLOW_ID_V36B,
        workflowFile: BACKGROUND_MASTER_SEEDVR_WORKFLOW_FILE_V36B,
        loadImageNodeId: BACKGROUND_MASTER_SEEDVR_LOAD_IMAGE_NODE_ID_V36B,
        saveImageNodeId: BACKGROUND_MASTER_SEEDVR_SAVE_IMAGE_NODE_ID_V36B,
        gpuTarget: BACKGROUND_MASTER_SEEDVR_GPU_TARGET_V36B,
      },
      received: {
        workflowId: body?.workflowId ?? body?.preset ?? null,
        preset: body?.preset ?? body?.workflowId ?? null,
        workflowFile: body?.workflowFile ?? null,
        workflowPath: body?.workflowPath ?? null,
        workflowJsonPath: body?.workflowJsonPath ?? null,
        workflowPresetPath: body?.workflowPresetPath ?? null,
        loadImageNodeId: body?.loadImageNodeId ?? body?.imageNodeId ?? null,
        saveImageNodeId: body?.saveImageNodeId ?? null,
        gpuTarget: body?.gpuTarget ?? null,
      },
    };
  }
...
function characterReferenceInternalWorkflowKindV1(body: any): "qwen" | "seedvr" | null {
  const key = workflowKey(body);
  if (key.includes("internal/character-reference/qwen_character_4angle_lowres")) return "qwen";
  if (key.includes("internal/character-reference/seedvr2_character_reference_1080p")) return "seedvr";
  return null;
}

function applyCharacterReferenceInternalInputV1(graph: any, body: any) {
  const kind = characterReferenceInternalWorkflowKindV1(body);
  if (!kind) return false;

  const imageA = typeof body?.imageA === "string" ? body.imageA : "";
  if (!imageA) {
    throw new Error("Character Reference workflow requires imageA/imageAPath.");
  }

  const nodeId = kind === "qwen" ? "25" : "1";
  if (graph?.[nodeId]?.class_type !== "LoadImage") {
    throw new Error(`Character Reference ${kind} workflow contract mismatch: node ${nodeId} must be LoadImage.`);
  }

  setNodeIfPresent(graph, nodeId, { image: imageA });
  return true;
}

function applyCharacterReferenceBackendModelsV1(
  graph: any,
  body: any,
  backendId: string | null
) {
  if (characterReferenceInternalWorkflowKindV1(body) !== "qwen") return;
  if (backendId !== "rtx3090") return;

  const replacements = [
    ["428:161", "UNETLoader", "unet_name", "qwen_image_edit_2511_int8_convrot.safetensors"],
    ["428:220", "LoraLoaderModelOnly", "lora_name", "Qwen-Image/Editing/qwen-image-edit-2511-multiple-angles-lora.safetensors"],
    ["428:153", "LoraLoaderModelOnly", "lora_name", "Qwen-Image/Acceleration/Qwen-Image-Edit-2511-Lightning-4steps-V1.0-bf16.safetensors"],
  ] as const;

  for (const [nodeId, classType, inputName, value] of replacements) {
    if (graph?.[nodeId]?.class_type !== classType || !graph?.[nodeId]?.inputs) {
      throw new Error(`Character Reference 3090 model contract mismatch at node ${nodeId}.`);
    }
    graph[nodeId].inputs[inputName] = value;
  }
}
...
export async function POST(req: NextRequest) {
  ...
  const backgroundMasterSeedvrContractError = assertBackgroundMasterSeedvrRequestV36B(body);
  if (backgroundMasterSeedvrContractError) {
    return Response.json(backgroundMasterSeedvrContractError, { status: 400 });
  }
  ...
  applyCharacterReferenceBackendModelsV1(graph, body, manifestSelection?.ok ? manifestSelection.backend.id : null);
  ...
  const seedRes = setSeedAuto(graph);
  ...
  try {
    ...
    if (error instanceof ComfyGpuBusyError) {
      return Response.json(
        { ok: false, error: error.message, code: error.code, gpu: error.gpu },
        { status: error.status },
      );
    }
