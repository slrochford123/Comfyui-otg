...
function normalizeWorkflowImage(value: unknown) {
  ...
  try {
    ...
    // Keep raw string and normalize below.
  }

  raw = decodeURIComponent(raw).replace(/\\/g, "/").trim();

  if (/^[A-Za-z]:\//.test(raw)) return path.win32.basename(raw);

  if (raw.startsWith("/")) {
    const viewFilenameMatch = raw.match(/[?&]filename=([^&]+)/i);
    if (viewFilenameMatch?.[1]) return decodeURIComponent(viewFilenameMatch[1]).trim();
...
async function scenePassFindLocalImageByBasenameV36BPG1(fileName: string) {
  ...
  const roots = scenePassCandidateRepoRootsV36BPG1().flatMap((root) => [
    path.join(root, "data"),
...
async function prepareScenePassReferencesForComfyV36BPG1(references: NormalizedReference[], requestOrigin: string): Promise<NormalizedReference[]> {
  ...
  for (const ref of references) {
    const workflowImage = await scenePassEnsureReferenceImageInComfyInputV36BPG1(ref.workflowImage, requestOrigin);
