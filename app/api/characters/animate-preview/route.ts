...
async function fetchResponse(url: string, init: RequestInit, timeoutMs: number) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
...
async function submitComfyPrompt(prompt: JsonRecord, clientId: string) {
  ...
  const response = await submitComfyPromptWith5060Lease({
    baseUrl: COMFY_URL,
    workerId: "character-animate-preview",
    init: {
      method: "POST",
    ...
    fetcher: (url, init) => fetchResponse(url, init, 120_000),
  });
