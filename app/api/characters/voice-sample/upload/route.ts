...
function extensionForFile(file: File) {
  ...
  if (isAllowedVoiceSampleExtension(ext)) return ext;
  return "";
}
...
async function resolveOwnerKey(req: NextRequest): Promise<string> {
  if (hasValidWorkerToken(req)) {
    const workerOwnerKey = String(
      req.headers.get("x-otg-owner-key") || "",
    ).trim();

    if (!isSafeVoiceSampleUploadSegment(workerOwnerKey)) {
      throw new Error(
        "Missing or invalid x-otg-owner-key for worker upload.",
      );
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rawCharacterId = String(
      form.get("characterId") || "character",
    ).trim();

    const rawJobId = String(form.get("jobId") || "").trim();

    const provider = String(
      form.get("provider") || (workerUpload ? "qwen3" : "uploaded"),
    ).trim();

    const adapter = String(
      form.get("adapter")
      || (workerUpload ? "windows_voice_design" : "uploaded_voice"),
    ).trim();
    ...
    if (
      workerUpload
      && !isSafeVoiceSampleUploadSegment(rawCharacterId)
    ) {
      return jsonError(
        "characterId is required and must be a safe path segment.",
      );
    }

    if (
      workerUpload
      && !isSafeVoiceSampleUploadSegment(rawJobId)
    ) {
      return jsonError(
        "jobId is required and must be a safe path segment for worker upload.",
      );
    }
    ...
    if (file.size <= 0) {
      return jsonError("Audio file is empty.");
    }
    ...
    if (!isAllowedMime(file)) {
      return jsonError("Unsupported audio MIME type.");
    }
    ...
    if (!ext) {
      return jsonError(
        "Unsupported audio extension. Use wav, mp3, m4a, flac, or ogg.",
      );
    }
    ...
    const uploadId = workerUpload
      ? safeSegment(rawJobId)
      : `uploaded_${randomUUID().replace(/-/g, "").slice(0, 12)}`;

    // Persist the actual validated source format. Provider selection must
    // never rename FLAC bytes to MP3 or otherwise mislabel the audio.
    const fileName = voiceSampleFileNameForExtension(ext);

    const voiceSamplesRoot = path.join(
      OTG_DATA_ROOT,
      "characters",
      ownerSegment,
      "voice-samples",
