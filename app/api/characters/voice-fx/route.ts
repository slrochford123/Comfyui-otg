...
function publicAudioUrl(filePath: string) {
  ...
  return (
    "/api/characters/voice-file?path=" +
    encodeURIComponent(relative.replace(/\\/g, "/"))
  );
}
...
export async function POST(request: Request) {
  try {
    ...
    const resolvedInputPath = path.isAbsolute(inputPath)
      ...
      : path.resolve(DATA_ROOT, normalizedInputPath);
    ...
    const params = {
      ...
      saturationAmount: Number(body.saturationAmount || 0),
      overdriveAmount: Number(body.overdriveAmount || 0),
      fuzzAmount: Number(body.fuzzAmount || 0),
