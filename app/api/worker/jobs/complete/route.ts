...
import { enqueueQwen3VoiceSampleLifecycleRelease } from "@/lib/workers/voiceLifecycleHooks";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const lifecycleCleanup = enqueueQwen3VoiceSampleLifecycleRelease(job, "complete");
    return NextResponse.json(
      {
        ok: true,
        job,
        profilePersisted,
        ...(lifecycleCleanup.enabled ? { lifecycleCleanup } : {}),
      },
      { headers: withNoStore() },
    );
  } catch (error) {
