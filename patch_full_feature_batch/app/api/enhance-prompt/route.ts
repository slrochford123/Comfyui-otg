...
function buildPrompt(mode: EnhanceMode, text: string, size: EnhanceSize, context?: string) {
  ...
  if (mode === "background") {
    return (
      ...
      "Keep the same setting and intent. Do NOT introduce new locations or objects. Make it vivid but compact. " +
      sizeInstr +
  ...
  return (
    ...
    "Preserve meaning and continuity. Improve clarity, detail, lighting, texture, and cinematic phrasing. " +
    sizeInstr +
    "STRICT RULES: Do NOT add new characters, new objects/props, new locations, new time-of-day, or new camera angles. " +
    "Do NOT change identity, clothing intent, or setting. " +
    "Keep the same subject(s), same action, same environment. " +
    "Return ONLY the improved prompt text (no markdown, no quotes, no commentary)." +
    ctxBlock +
    `Scene: ${text}`
  );
