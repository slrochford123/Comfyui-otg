...
if (!exists && filtered[0]) setWorkflowId(filtered[0].id);
  ...
  // RECENT_PROMPTS_V1: load recent positive prompts from localStorage
  useEffect(() => {
    try {
      const raw = localStorage.getItem("otg:recentPositivePrompts:v1");
      if (raw) {
        const arr = JSON.parse(raw);
        if (Array.isArray(arr)) setRecentPrompts(arr.filter((x) => typeof x === "string").slice(0, 5));
      }
    } catch {
      // ignore
    }
  ...
  const pushRecentPrompt = useCallback((t: string) => {
    const s = String(t || "").trim();
    if (!s) return;
    setRecentPrompts((prev) => {
      const dedup = [s, ...prev.filter((x) => String(x) !== s)];
      const next = dedup.slice(0, 5);
      try {
        localStorage.setItem("otg:recentPositivePrompts:v1", JSON.stringify(next));
      } catch {}
      return next;
    });
  }, []);
...
const onGenerate = useCallback(async () => {
  ...
  try {
    ...
    try { pushRecentPrompt(positivePrompt); } catch {}
...
const onClearPreview = useCallback(async () => {
  ...
  // Gallery -> Edit Image (Qwen Image Edit)
  const [editModal, setEditModal] = useState<null | { name: string; url: string }>(null);
  const [editPositive, setEditPositive] = useState<string>("");
  ...
  const submitAnimate = useCallback(async () => {
    ...
    try {
      ...
      // LTX2_ORIENTATION_LOCK_V1: match target video orientation to source image.
      let isPortrait = false;
      try {
        const bmp: any = await (globalThis as any).createImageBitmap(blob);
        const w = Number(bmp?.width || 0);
        const h = Number(bmp?.height || 0);
        if (w > 0 && h > 0) isPortrait = h > w;
        try { bmp.close?.(); } catch {}
      } catch {
        // ignore
      }
      ...
      if (graph?.["102"]?.inputs) {
        graph["102"].inputs["resize_type.width"] = isPortrait ? 720 : 1280;
        graph["102"].inputs["resize_type.height"] = isPortrait ? 1280 : 720;
      }
  ...
  const submitEditImageFromGallery = useCallback(async () => {
    if (!editModal) return;
    const pos = String(editPositive || "").trim();
    if (!pos) {
      setEditErr("Positive edit instruction is required.");
      return;
    }
    ...
    try {
      ...
      // TextEncode nodes
      if (graph?.["442"]?.inputs) graph["442"].inputs.prompt = "__OTG_POSITIVE_PROMPT__";
      if (graph?.["440"]?.inputs) graph["440"].inputs.prompt = "__OTG_NEGATIVE_PROMPT__";

      // Randomize seed each edit (node 447: KSampler seed)
      try {
        if (graph?.["447"]?.inputs) graph["447"].inputs.seed = Math.floor(Math.random() * 2147483640) + 1;
      } catch {}

      const fd = new FormData();
      fd.append("imageA", file, file.name);
      fd.append("positivePrompt", pos);
      fd.append("negativePrompt", String(editNegative || "").trim());
      fd.append("promptJson", JSON.stringify(graph));
      fd.append("client_id", getOrCreateDeviceId());
      if (isAdmin && comfyTargetId) fd.append("comfyTargetId", String(comfyTargetId));
      ...
      // Bubble queue widget
      try {
        const pid = String((j.data as any)?.prompt_id || "").trim();
        if (pid) fq.add({ id: pid, title: `Edit: ${editModal.name}`, status: "queued" });
      } catch {}

      showToast("Edit submitted");
      setEditModal(null);
      setEditPositive("");
      setEditNegative("");
      await loadGallery();
  ...
  }, [editModal, editPositive, editNegative, getOrCreateDeviceId, isAdmin, comfyTargetId, loadGallery]);
          ...
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <Card title="Prompt">
              <textarea
                ...
                onChange={(e) => { setEnhanceUndo(null); setPositivePrompt(e.target.value); }}
                rows={5}
                placeholder="Describe what you want..."
                className="otg-textarea"
              />
              <div className="otg-row" style={{ marginTop: 8, gap: 10, flexWrap: "wrap", justifyContent: "space-between" }}>
                <button
                  type="button"
                  className="otg-btnGhost"
                  disabled={uiLocked || !enhanceUndo}
                  onClick={() => {
                    if (!enhanceUndo) return;
                    setPositivePrompt(enhanceUndo);
                    setEnhanceUndo(null);
                  }}
                  title="Undo the last Enhance Prompt"
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (v) setPositivePrompt(v);
                    e.currentTarget.value = "";
                  }}
                  disabled={uiLocked || recentPrompts.length === 0}
                  style={{ flex: 1, minWidth: 180 }}
                  title="Recent prompts (last 5)"
                >
                  <option value="">{recentPrompts.length ? "Recent prompts…" : "No recent prompts"}</option>
...
{tab === "gallery" ? (
                ...
                {galleryFiles.map((f) => (
                  ...
                  >
                    ...
                    <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      <div className="otg-row" style={{ flexWrap: "wrap", gap: 8 }}>
                        {!isVideoName(f.name) ? (
                          <PrimaryButton
                            type="button"
                            disabled={uiLocked}
                        ...
                        >
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M3 12a9 9 0 0 1 15-6.4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                            <path d="M18 3v5h-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            <path d="M21 12a9 9 0 0 1-15 6.4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                        ...
                        <button
                          ...
                          onClick={() => addToFavoritesFromGallery(f.name)}
                          disabled={uiLocked}
                          style={{ width: 40, height: 40, borderRadius: 999, display: "inline-flex", alignItems: "center", justifyContent: "center" }}
                          aria-label="Favorite"
                        >
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 21s-7-4.4-9.5-8.5C.6 9 2.1 6 5.5 6c1.7 0 3 .9 3.7 2 0.7-1.1 2-2 3.8-2C16.4 6 18 9 21.5 12.5 19 16.6 12 21 12 21Z" stroke="currentColor" strokeWidth="2" strokeLinejoin="round"/>
                          </svg>
                        </button>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              setErr(null);
                              const oldName = String(f.name || "");
                              const ext = oldName.includes(".") ? oldName.slice(oldName.lastIndexOf(".")) : "";
                              const base = ext ? oldName.slice(0, -ext.length) : oldName;
                              const nextBase = window.prompt("Rename file (without extension):", base);
                              if (!nextBase) return;
                              const res = await fetch(`/api/gallery/rename`, {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({ name: oldName, newBase: String(nextBase) }),
                              });
                              const j = await res.json().catch(() => null);
                              if (!res.ok || !j?.ok) throw new Error(j?.error || `Rename failed (${res.status})`);
                              await loadGallery();
                            } catch (e: any) {
                              setErr(String(e?.message || e));
                            }
                        ...
                        <button
                          type="button"
                          onClick={() => deleteFromGallery(f.name)}
                          disabled={uiLocked}
      ...
      {editModal ? (
        <div
          role="dialog"
          aria-modal="true"
          onClick={() => { if (!editBusy) setEditModal(null); }}
          style={{
            position: "fixed",
            inset: 0,
            background: "rgba(0,0,0,0.72)",
        ...
        >
          ...
          >
            <div className="otg-row" style={{ justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button type="button" className="otg-btnGhost" onClick={() => { if (!editBusy) setEditModal(null); }}>Close</button>
            </div>

            <div style={{ marginTop: 12 }}>
              <img style={{ width: "100%", borderRadius: 16, display: "block" }} src={editModal.url} alt={editModal.name} />
            </div>
            ...
            <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
              <div>
                ...
                <textarea
                  ...
                  placeholder='e.g., "change the fish to black", "make the armor silver", "remove the logo"'
                  rows={4}
                  disabled={editBusy}
                />
              </div>
              ...
              {editErr ? <div className="otg-error">{editErr}</div> : null}

              <div className="otg-row" style={{ justifyContent: "flex-end" }}>
                <button type="button" className="otg-btnPrimary" onClick={submitEditImageFromGallery} disabled={editBusy || uiLocked}>
                  {editBusy ? "Submitting..." : "Submit Edit"}
                </button>
              </div>
            </div>
          </div>
