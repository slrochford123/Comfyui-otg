...
- [x] PROD WAN runtime acceptance passed on sealed release `/opt/otg/releases/prod-wan-7e986b4-20260817T021844Z`, commit `7e986b480f7575eddc02a2ef6bcded3fd41e9129`, tree `3825e0634d1613b223f36ea5bfc42d672c84a339`; Step 1 produced `WAN22_text_to_video_gguf_00007.mp4` as H.264 1280x720 at 24 FPS, 121 frames / 5.041667 s, SHA256 `b37393dcb6f3a1c0dc9a1ad8dbd2c0295edb1b4089e048a21afe1f5d867c1095`.
- [x] PROD concurrent cross-node acceptance passed: while Shawn RTX 3090 WAN video prompt `4661a263-57f1-4d21-99b2-09cf96424379` was running, SLR RTX 5060 Ti image prompt `c6797922-7188-42ac-b956-94b7bd85e6cf` completed successfully. The image was verified only in SLR Comfy history and saved as a 512x512 RGB PNG with SHA256 `2ff8a9f041652bc2391011efe2f1fcc7f534700fc37fc61eafd8472afa6cf746`; the concurrent video completed as H.264 1280x720 at 24 FPS, 121 frames / 5.041667 s, SHA256 `06cbd00f6e5256268e95b03f84adddec39a42333087d736e72ea4116291f236c`.
- [x] Whole-cluster PROD acceptance passed after concurrency: `otg-prod.service` remained active/running with `NRestarts=0`, local/public PROD and TEST health returned HTTP 200, both Comfy queues were empty, both Qwen `/api/ps` endpoints were empty, WorkerManager reported `locks: []` with SLR image and Shawn video lanes available, and the current PROD invocation journal scan passed.
- [x] Safer r3 sealed-release activation path is proven in PROD with fail-closed journal checks, service-stability verification, preserved rollback target, and automatic rollback trap semantics.
- [x] Step-5 source cleanup removes stale active Windows WorkerManager catalog ownership for `comfy-3090-sage-video`, `xtts`, `bg-remove`, `whisper`, `speaker-diarization`, and `ace-step`, retires the obsolete `gpu:windows-3090` / `comfy:windows-3090` active lock contract, and keeps legacy Windows scripts disconnected rather than deleting unrelated voice/audio feature code.
- [x] Step-5 video submission fallback semantics now preserve deterministic primary Comfy `/prompt` 4xx rejections unchanged and do not replay them cross-GPU; only eligible server-side rejection paths can proceed to the existing verified fallback flow, while ambiguous/network submissions retain duplicate-prevention no-retry behavior.
- [x] Step-5 source gate passed under Node 20.20.2 / ABI 115: 104 Vitest files / 741 tests, TypeScript, and `git diff --check` all passed.

### Remaining
- [x] Deploy the scoped Step-5 WorkerManager cleanup and video-submission fallback correction to TEST only, then validate the active WorkerManager catalog/status plus deterministic primary 4xx passthrough. COMPLETE 2026-08-17: TEST release `step5-worker-fallback-1325011-20260817T033409Z` at commit `132501163d699ffc4d670219ae6bc1aa3f03cb2c` passed authenticated WorkerManager runtime validation with both Linux lanes available, no locks, and no stale active IDs. The deterministic nonexistent-model Comfy validation returned app HTTP 400 with `upstreamStatus: 400` and left the Comfy queue empty. No repeat WAN acceptance render was required.
- [x] If Step-5 TEST runtime validation passes, promote only the scoped cleanup/fallback changes to PROD using the sealed-release/r3 activation path, then verify service health, WorkerManager status, and error semantics without another heavy render. COMPLETE 2026-08-17: sealed PROD release `prod-step5-1325011-20260817T040115Z` is live at commit `132501163d699ffc4d670219ae6bc1aa3f03cb2c`, tree `22cfa7b0abeafba88761bb684ecdc373ccee65d0`, with rollback target `prod-wan-7e986b4-20260817T021844Z`. r3 activation succeeded; `otg-prod.service` remained active with `NRestarts=0`; local and public health returned 200. WorkerManager reported both Linux lanes available and no locks; legacy `comfy-3090-sage-video`, `ace-step`, and `bg-remove` strings were present only in historical `commands` audit records, not active catalog/lock/lane state. Deterministic PROD nonexistent-model validation returned HTTP 400 with `upstreamStatus: 400` and node 102 validation evidence while both Comfy queues remained empty.
- [ ] Dependency-hardening follow-up remains open: PROD packaging reported `npm audit` findings of 12 vulnerabilities (1 low, 8 high, 3 critical). Runtime acceptance passed, but these findings must be handled separately through controlled dependency updates and regression testing; do not mark them resolved by the runtime pass.
...
- [x] TEST env loading was migrated from `/home/shawn-rochford/AI/work/OTG-Test2/.env.local` to `/home/shawn-rochford/AI/runtime/test/config/app-env/.env.local`. The live TEST service resolves `OTG_WORK_REPO` to that persistent env root; Qwen3, CosyVoice, and Blender helper paths resolve through the active TEST release. `OTG-Test2` is no longer a live TEST runtime dependency, but the tree must currently be retained because `/home/shawn-rochford/AI/work/OTG-Test2/.git` is the shared Git common directory for linked worktrees.

<!-- OTG_TEST_RUNTIME_DURABLE_ACCEPTED_20260813 -->
- [x] Remaining live `OTG-Test2` systemd dependencies were removed on 2026-08-13: active TEST and Character worker unit files were cleaned, the inactive/disabled CosyVoice and LTX `-3003` units were retired, TEST remained HTTP 200 with `NRestarts=0`, all seven active Character workers remained healthy, and PROD remained HTTP 200 without mutation.
- [x] Clean source-controlled TEST release `test-runtime-durable-20260813T110743Z`, built from published branch `publish/test-runtime-durability-20260813`, passed the full hardened payload contract and was transactionally activated on 2026-08-13. After activation, TEST remained HTTP 200 with `NRestarts=0`, all seven Character workers remained active, startup logs contained no `EADDRNOTAVAIL` or `@next/env` regression, PROD remained HTTP 200, and manual browser validation passed across the current 3001 interface. E003 remains preserved as the immediate rollback release.

## 2026-08-16 two-node TEST runtime validation follow-up

- [x] Explicitly unloaded stale Shawn `qwen3.6:27b` residency through the audited Ollama API after proving Qwen Code and app ownership absent; Ollama stayed alive, `/api/ps` emptied, and Shawn VRAM returned toward idle.
- [x] Shawn video smoke succeeded once through the application GPU-lease path: prompt `6066ab3d-2fbd-461c-bbc5-1d853119f40d`, output `OTG-runtime-validation/phase9-shawn-video-1786888066432_00001.mp4`, SHA256 `a59cc1634bd1c1f4198fd8f1bc5ad5540e4859ef254c468c2840000cc7ed1c7c`.
- [x] Concurrent independent-lane proof succeeded: SLR image prompt `f5408573-7818-419b-b865-adf40eb64cf1` and Shawn video prompt `4cb7da1f-6b84-4895-8141-b2f3a48e75a3` ran simultaneously with their distinct leases, one POST each, terminal success, and clean release.
- [x] Live Qwen routing A/B/C/D passed: short context preferred SLR, an SLR lock routed short context to Shawn, long context selected Shawn only, and both locked produced a clear busy result with zero Ollama POSTs; all app-loaded models unloaded afterward.
- [ ] Live video fallback generation remains pending because the current capability manifest contains no small video workflow verified compatible on both nodes. WAN video is Shawn-only, while the nominal LTX-compatible workflow is missing its required model on SLR. Controlled failover, lock ordering, ambiguous-POST, and duplicate-prevention contracts pass 63/63.
- [x] Applio claim source defect repaired: `character_voice_pipeline/test_trained_voice` all-owner claim is permitted only for `linux-applio-inference-worker`; an unrelated worker and an unrelated universal action remain rejected. Focused voice-pipeline regression passes 58/58.
- [x] WorkerManager status root cause identified: canonical persistent TEST env incorrectly retained `OTG_WORKER_CONTROL_ENABLED=0`, despite the installed authenticated lifecycle agent and prior TEST control-plane contract. The TEST builder now fails closed unless control is enabled and a bearer token exists; defaults remain disabled elsewhere.
- [x] Live closure of WorkerManager status and Applio HTTP 400 completed in Phase 15 after transactional activation of the follow-up TEST release; authenticated status and post-activation worker logs both pass live.
- [x] PROD was not queried, mutated, restarted, or deployed during this follow-up.

## 2026-08-16 Phase 15 FINAL ACCEPTANCE — TEST only

- [x] Transactional activation identity is correct: `/home/shawn-rochford/AI/deploy/otg-test/current` resolves to `/home/shawn-rochford/AI/deploy/otg-test/releases/two-node-cluster-runtime-repair-20260816T135943Z`, whose `PREVIOUS_RELEASE.txt` points to `/home/shawn-rochford/AI/deploy/otg-test/releases/two-node-cluster-gpu-arbitration-20260816T123647Z`.
- [x] TEST application acceptance passed after the 2026-08-16 10:05:04 EDT activation: `otg-test.service` is active/running with `Result=success`, `NRestarts=0`, and `/api/healthz` returns HTTP 200. Post-ready logs contain no `EADDRNOTAVAIL`, `@next/env`, crash, restart, NVIDIA Xid, or OOM-kill regression.
- [x] All seven enabled Character workers are active/running with `Result=success` and stable `NRestarts=0`: Applio inference, Applio training, completion, CosyVoice, IndexTTS2 dataset, preview, and Qwen3 voice. Activation-time connection races stopped once the TEST listener became ready; no post-ready worker failure or restart remains.
- [x] WorkerManager live defect closed: unauthenticated `/api/worker-control/status` returns HTTP 401, while a request using the effective service bearer credential returns HTTP 200 with `ok=true`, control enabled, real actions available, and sanitized `slrImage` / `shawnVideo` lane states. The live response scan found no bearer/token value, local filesystem path, raw command line, shell/PowerShell executable, or WorkerManager script path exposure; only bounded operational status fields are returned.
- [x] Applio inference live defect closed without creating a training or inference job: the pre-activation journal contained 1,500 repeated HTTP 400 claim failures, while the post-ready journal contains zero HTTP 400 matches and at least 44 successful idle universal claims for `character_voice_pipeline/test_trained_voice` by `linux-applio-inference-worker`.
- [x] Final two-node health passed: WorkerManager reports zero active locks, including no stale `gpu:slr-5060` or `gpu:shawn-3090`; both SLR and Shawn Comfy queues are idle; both Qwen `/api/tags` endpoints return HTTP 200; SLR `/api/ps` is empty and Shawn's `qwen3.6:27b` residency is accounted for by two live Qwen Code processes rather than a stale app claim; `/mnt/otg-cluster` passed a create/read/remove write probe; and both hosts show no new NVIDIA Xid or OOM-kill evidence.
- [x] Comfy/runtime stability passed: Shawn `otg-comfyui.service` is active/running with `NRestarts=0`; SLR Comfy is online under `pm2-slrochford123.service` with PM2 restart count 0; neither Comfy runtime nor TEST recorded a post-ready crash/restart.
- [x] Phase 8-12 image, video, concurrent-lane, and Qwen routing evidence is carried forward from the immediately preceding validation because this repair changed only WorkerManager enablement/status packaging and the Applio universal-claim policy. The expensive routing tests were not rerun.
- [x] Read-only PROD final verification passed: canonical PROD remains on `slr`; `/opt/otg/current-prod` is unchanged at `/opt/otg/releases/prod-current3003-75d65980-20260813T011142`; `otg-prod.service` is active/running with `Result=success` and `NRestarts=0`; direct and public `https://comf-otg.comfyui-otg.win/api/healthz` probes return HTTP 200; its service start predates this TEST activation and no PROD restart or mutation is attributable to it.
