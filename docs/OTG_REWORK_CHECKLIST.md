...
### Remaining
- [ ] Deploy the scoped Step-5 WorkerManager cleanup and video-submission fallback correction to TEST only, then validate the active WorkerManager catalog/status plus deterministic primary 4xx passthrough. Use non-rendering or cheap validation; do not repeat the expensive WAN acceptance render.
- [ ] If Step-5 TEST runtime validation passes, promote only the scoped cleanup/fallback changes to PROD using the sealed-release/r3 activation path, then verify service health, WorkerManager status, and error semantics without another heavy render.
