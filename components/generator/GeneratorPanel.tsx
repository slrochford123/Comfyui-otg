...
async function stopAndCleanupRecorder() {
  try {
    mediaRecorderRef.current?.stop();
  } catch {
    // ignore
...
async function handleMicToggle() {
  ...
  if (!navigator?.mediaDevices?.getUserMedia) {
    setMicMsg("Microphone not supported in this browser.");
    return;
  }
  ...
  try {
    ...
    rec.onstop = async () => {
      ...
      try {
        ...
        const r = await fetch("/api/ollama-ai/transcribe", { method: "POST", body: fd });
        const j = await r.json().catch(() => null);

        if (!r.ok || !j?.ok) {
          setMicMsg(String(j?.error || "Transcription failed."));
          return;
        }

        const text = String(j?.text || "").trim();
        if (!text) {
          setMicMsg("No speech detected.");
          return;
        }

        const a = String(positivePrompt || "").trim();
setPositivePrompt(a ? `${a} ${text}` : text);
      } catch (e: any) {
        setMicMsg(String(e?.message || e || "Transcription failed."));
      } finally {
    ...
    setMicMsg(String(e?.message || e || "Microphone permission denied."));
    await stopAndCleanupRecorder();
  }
}
...
<div className="otg-row otg-between otg-center">
  ...
  <button
    ...
    title={recording ? "Stop recording" : "Speak prompt"}
    aria-label={recording ? "Stop recording" : "Speak prompt"}
    style={{
      padding: "8px 10px",
      minWidth: 40,
      borderRadius: 999,
      border: recording ? "1px solid rgba(168,85,247,0.9)" : undefined,
      boxShadow: recording ? "0 0 0 2px rgba(59,130,246,0.25)" : undefined,
    }}
  >
    {/* Microphone icon */}
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      />
        ...
        {micMsg ? (
          <div className="otg-muted" style={{ marginTop: 8 }}>{micMsg}</div>
        ) : null}
