...
def build_plan(args: argparse.Namespace, owner_key: str, character_id: str, job_id: str, job_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    python = Path(args.applio_python).resolve()
    ...
    output_dir = Path(args.repo) / "data" / "characters" / safe_segment(owner_key) / "applio-models" / safe_segment(character_id) / safe_segment(job_id)
    logs_dir = output_dir / "logs"
    datasets_root = Path(clean(os.environ.get("APPLIO_DATASETS_ROOT")) or (Path(args.repo) / "data" / "applio" / "datasets"))
...
def run_training(args: argparse.Namespace, job: Dict[str, Any]) -> None:
    ...
    artifact = {
        ...
        "note": "Real Applio model trained by the dedicated Windows Applio worker.",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Dedicated OTG Windows Applio training worker.")
    ...
    parser.add_argument("--worker-id", default=os.environ.get("OTG_WORKER_ID", "windows-voice-applio-worker"))
    parser.add_argument("--worker-token", default=os.environ.get("OTG_WORKER_TOKEN", ""))
    parser.add_argument("--applio-root", default=os.environ.get("APPLIO_ROOT", r"C:\AI\Voices\Applio"))
    parser.add_argument("--applio-python", default=os.environ.get("APPLIO_PYTHON", r"C:\AI\Voices\Applio\env\python.exe"))
    parser.add_argument("--applio-core", default=os.environ.get("APPLIO_CORE_SCRIPT") or os.environ.get("APPLIO_TRAIN_SCRIPT") or r"C:\AI\Voices\Applio\core.py")
    ...
    log("Starting dedicated OTG voice Applio worker")
