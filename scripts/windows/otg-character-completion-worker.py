...
def complete_job(args: argparse.Namespace, owner_key: str, job_id: str, result: Dict[str, Any]) -> None:
    request_json(
        ...
        {
            "jobId": job_id,
            "message": "Worker Manager completed the character card and saved the character.",
            "result": result,
        },
        timeout=120,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="OTG Worker Manager adapter for durable character completion")
    ...
    parser.add_argument("--device-id", default=os.environ.get("OTG_DEVICE_ID", "windows-character-completion"))
    parser.add_argument("--worker-id", default=os.environ.get("OTG_WORKER_ID", "windows-character-completion-worker"))
    ...
    if not clean(args.worker_token):
        raise RuntimeError("Missing OTG_WORKER_TOKEN. The token must be inherited through the Worker Manager environment.")
