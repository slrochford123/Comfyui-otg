...
REM SLR Studios OTG - Qwen3-TTS API launcher - v3 native DLL PATH hardening
...
set "CONDA_ROOT=C:\Users\SLRoc\miniconda3"
...
if not exist "%QWEN3_VENV_PY%" (
  echo [FAIL] Missing Qwen3 venv Python: %QWEN3_VENV_PY%
  pause
  exit /b 1
)
...
REM Harden native DLL search path.
if exist "%CONDA_ROOT%" (
  set "PATH=%CONDA_ROOT%;%CONDA_ROOT%\DLLs;%CONDA_ROOT%\Library\bin;%CONDA_ROOT%\Scripts;%CONDA_ROOT%\condabin;%PATH%"
)
...
echo [qwen3-tts-api] Conda root: %CONDA_ROOT%
...
echo [qwen3-tts-api] Checking exact API imports...
"%QWEN3_VENV_PY%" -X faulthandler -c "import torch, soundfile, fastapi, uvicorn; print('[OK] basics'); from qwen_tts import Qwen3TTSModel; print('[OK] Qwen3TTSModel primary import')"
if errorlevel 1 (
  echo [WARN] Primary Qwen3TTSModel import failed. Trying fallback import...
  "%QWEN3_VENV_PY%" -X faulthandler -c "from qwen_tts.inference.qwen3_tts_model import Qwen3TTSModel; print('[OK] Qwen3TTSModel fallback import')"
  if errorlevel 1 (
    echo [FAIL] Qwen3TTSModel import crashed/failed.
    echo Run diagnose_qwen3_native_crash.ps1 and paste the first failing section.
    pause
    exit /b 1
  )
)
...
if "%ERR%"=="-1073741515" (
  echo [qwen3-tts-api] Meaning: 0xC0000135 / STATUS_DLL_NOT_FOUND. A native DLL dependency is missing.
)
