...
backup_dir = root / ".manual-backups" / ("character-voice-fx-ui-" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
# 1. Add Voice FX type.
# -----------------------------
voice_settings_end = """type VoiceSettings = {
...
if "type VoiceFxSettings" not in text:
    if voice_settings_end not in text:
        raise RuntimeError("Missing VoiceSettings type anchor.")
    text = text.replace(voice_settings_end, voice_settings_end + "\n" + voice_fx_type, 1)

# -----------------------------
# 2. Add default FX settings and presets.
# -----------------------------
...
if "setVoiceFx(DEFAULT_VOICE_FX)" not in text:
    if old_reset not in text:
        raise RuntimeError("Missing reset voicePreview anchor.")
    text = text.replace(old_reset, new_reset, 1)

# -----------------------------
# 5. Add functions after generateVoicePreview block.
# -----------------------------
...
voice_fx_functions = """  function setVoiceFxField<K extends keyof VoiceFxSettings>(key: K, value: VoiceFxSettings[K]) {
    setVoiceFx((current) => ({ ...current, [key]: value, preset: key === "preset" ? value as VoiceFxSettings["preset"] : current.preset }));
  }
...
# -----------------------------
# 6. Insert Voice FX UI before the Voice Lab action buttons.
# -----------------------------
voice_panel_marker = '<Panel title="Voice Lab">'
panel_pos = text.find(voice_panel_marker)
if panel_pos == -1:
    raise RuntimeError("Missing Voice Lab panel marker.")
...
voice_fx_ui = '''              <div className="mt-5 rounded-xl border border-cyan-400/30 bg-cyan-400/5 p-4">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-3">
                  ...
                  <label className="block text-sm text-zinc-300">
                    <span className="mb-1 block text-xs uppercase tracking-[0.16em] text-zinc-500">Gain dB</span>
                    <input
                      type="number"
                      min={-12}
                      max={12}
                      step={1}
                      value={voiceFx.gainDb}
                      onChange={(event) => setVoiceFxField("gainDb", Number(event.target.value))}
                      className="w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100"
                    />
                  </label>
